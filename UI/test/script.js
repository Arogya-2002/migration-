...
const emotionLabels = ["Happy", "Sad", "Angry", "Surprised", "Fearful", "Neutral"];
...
function addActorContainer() {
    ...
    container.innerHTML = `
        ...
        <label for="name-${actorCount}">Name:</label><br>
        <input type="text" id="name-${actorCount}" name="actors[${actorCount}]name" oninput="showSuggestions(this.value, ${actorCount}, 'actor')" autocomplete="off">
        <div id="suggestions-${actorCount}" class="suggestions"></div><br><br>
        
        <label for="dob-${actorCount}">Date of Birth (yyyy-mm-dd):</label><br>
        <input type="date" id="dob-${actorCount}" name="actors[${actorCount}]dob"><br><br>
        
        <label for="filmography-${actorCount}">Filmography:</label><br>
        <input type="checkbox" id="movie1-${actorCount}" name="actors[${actorCount}]filmography" value="hero">hero<br>
        <input type="checkbox" id="movie2-${actorCount}" name="actors[${actorCount}]filmography" value="heroine">heroine<br>
        <input type="checkbox" id="movie3-${actorCount}" name="actors[${actorCount}]filmography" value="comedian">comedian<br>
        <input type="checkbox" id="movie4-${actorCount}" name="actors[${actorCount}]filmography" value="villain">villain<br>
        <input type="checkbox" id="movie5-${actorCount}" name="actors[${actorCount}]filmography" value="character_artist">character_artist<br>
        <input type="checkbox" id="movie6-${actorCount}" name="actors[${actorCount}]filmography" value="child_artist">child_artist<br>
        <input type="checkbox" id="movie7-${actorCount}" name="actors[${actorCount}]filmography" value="cameo_role">cameo_role<br>
        <input type="checkbox" id="movie8-${actorCount}" name="actors[${actorCount}]filmography" value="director">director<br>
        <input type="checkbox" id="movie9-${actorCount}" name="actors[${actorCount}]filmography" value="writer">writer<br>
        <input type="checkbox" id="movie10-${actorCount}" name="actors[${actorCount}]filmography" value="screenplay_writer">screenplay_writer<br>
        <input type="checkbox" id="movie11-${actorCount}" name="actors[${actorCount}]filmography" value="story_writer">story_writer<br>
        <input type="checkbox" id="movie12-${actorCount}" name="actors[${actorCount}]filmography" value="dialogue_writer">dialogue_writer<br><br>
...
function getSelectedFilmography(actorIndex) {
    ...
    // Return the selected films as an array
    return selectedFilmography;
}
...
function getSelectedGenres(movieIndex) {
    ...
    return selectedGenres.join(', ');  // Return the selected genres as a comma-separated string
}
...
window.onload = function() {
    ...
    // Generate genre radio buttons for the movie container
    generateGenreOptions(1);
};
...
const movieGenres = Array.from(document.querySelector('#genre-1').selectedOptions)
    .map(option => option.value).join(',') || '';  // Join genres as comma-separated string or empty string if none selected
...
document.querySelectorAll('.dialog-container').forEach((container, index) => {
    ...
    const supabaseApiKey = 'eyJhbGciOiJIUzI1NiIsImtpZCI6IldsZFhxUFZFZndGbGJrQU4iLCJ0eXAiOiJKV1QifQ..dM0M49HJSQbQD4Ep8fnrCZxchAOGUd4NhYZfz9BWsf8';
    fetch(supabaseUrl, {
        method: 'POST',
        headers: {
